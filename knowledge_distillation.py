...
class KnowledgeDistillationTrainer(Trainer):                            
  ...
  def compute_loss(self, model, inputs, return_outputs=False):
   ...
   loss_kd=self.args.temperature**2 * distillation_loss(F.softmax(student_logits/self.args.temperature,dim=1),F.softmax(teacher_logits/self.args.temperature,dim=1))
...
teacher_model=AutoModelForCausalLM.from_pretrained("gpt2-xl").to(device="cuda")   #Necessary to push models to gpu environment for faster training



gpt2_trainer=KnowledgeDistillationTrainer(model_init=student_init,
