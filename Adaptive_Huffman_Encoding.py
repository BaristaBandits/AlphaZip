...
def sibling_property_test(
    node,
):  # Checks sibling property and swaps left and right children if not satisfied
...
def findflow(node):
    ...
    while parent != None:
        if node.weight > parent.weight:
            ...
            parent_parent, parent_left, parent_right = (
                parent.parent,
                parent.lchild,
                parent.rchild,
            )
            node.parent, node.lchild, node.rchild = (
                parent_parent,
                parent_left,
                parent_right,
            )
            parent.parent, parent.lchild, parent.rchild = (
                node_parent,
                node_left,
                node_right,
            )
...
def AdaHuffEncode(input_array):
    ...
    output = ""
    NYT = Node(
        "NYT", None, None, None, 0
    )  # Always the NYT Node corresponds to NODE ORDER 0
    for i in input_array:
        updated_flag = (
            0  # This flag helps us to identify if a symbol was updated or not
        )
        if (
            i not in stored_dict
        ):  # Condition True when the symbol is appearing for the first time in the tree
            updated_flag = 1
            ...
            if NYT.parent == None:  # This means that this is root node
                ROOT_NODE = Node(
                    "N" + str(count), None, NYT, setup_node, 1
                )  # Setup root node
                NYT.parent = stored_dict[i].parent = (
                    ROOT_NODE  # Map parents to the new node and NYT node
                )

            else:
                parent = NYT.parent
                replace_node = Node(
                    "N" + chr(count), parent, NYT, stored_dict[i], 0
                )  # Spanning the NYT node into two nodes: left-NYT and right- Symbol
                parent.lchild = NYT.parent = stored_dict[i].parent = (
                    replace_node  # Updating the flow of the tree
                )
    ...
    while index < len(bitstream):
        ...
        if current == NYT:  # Decoding the symbol that is a new symbol
            ...
            if NYT.parent == None:
                ...
                parent = NYT.parent
                replace_node = Node(
                    "N" + str(count), parent, NYT, restore_dict[symbol], 0
                )
                parent.lchild = NYT.parent = restore_dict[symbol].parent = replace_node
            current = ROOT_NODE

        elif (
            current.lchild == None and current.rchild == None
        ):  # Meaning we reached the symbol that is already added to the Tree
            symbol = current.tag
            Decode_output += symbol
            current = ROOT_NODE
