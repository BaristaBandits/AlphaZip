...
def run_test(model,tokenizer, test, context):                #Test: input text, sequence:context
  ...
  for i in range(len(right_ids)):
   ...
   inputs=tokenizer(f'{context}',return_tensors="tf")           #Context window moved forward
...
def extensive_test(filename,model_path):
  ...
  with open(filename,'r') as f:
   test=f.read(input_length)                                  #Constraining the input to first n characters (where n=input_length)
  ...
  #Rank compression
  gzip_compression =gzip.compress(encoded.encode('utf-8'))                  #GZIP BASED RANK COMPRESSION
